...
var go = new Go({
  ...
  go.execute({
    ...
    if (result.timeout) {
      ...
      return console.log('The \'delay me\' command timed out!\n\n');
    }
    return console.log('Go responded: ' + response.text);
  }, {
    commandTimeoutSec: 1
  ...
  go.execute({
    ...
    if (result.terminated) {
      ...
      return console.log('The \'crash me\' command was terminated!\n\n');
    }
    return console.log('Go responded: ' + response.text);
  });
  ...
  go.execute({
    ...
    if (!result.ok) { // result.ok is set iff everything went well
      return console.log('Something went wrong with the \'respond me\' command!');
    }
